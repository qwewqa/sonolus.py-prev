...
def download_server(
    ...
    *,
    ...
    categories: Collection[DownloadCategory] = (
        "levels",
        "skins",
        "backgrounds",
        "effects",
    ...
    for category in categories:
        ...
        if limit == "homepage":
            ...
            while True:
                ...
                page += 1
                if (limit is not None and len(items) >= limit) or (
                    page >= page_items.pageCount
                ):
                    break
        ...
        for item in items:
            try:
                details: dict[str, ItemDetails] = {
                    l: get_item(server, item.name, localization=l)
                    for l in localizations
                }
                bundles = [
                    bundle_type.from_item(
                        server, d.item, localization=l, description=d.description
                    )
